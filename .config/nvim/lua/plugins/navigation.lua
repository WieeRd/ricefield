    ...
    -- fuzzy finder: FZF integration
    {
        ...
        init = function(_)
            ...
            vim.ui.select = function(...)
                require("fzf-lua").register_ui_select(FZF_DROPDOWN)
        ...
        opts = {
            ...
            float = {
                ...
                override = function(conf)
                    ...
                    conf.width = math.max(math.floor(col * 0.66 - 2), 120)
                    conf.height = math.min(math.floor(row * 0.66 - 2), row - 6)
                    conf.col = (col - conf.width) / 2 - 1
                    conf.row = (row - conf.height) / 2 - 1
                    return conf
                end,
            },
    ...
    -- floating terminal window: quick shell access
    {
        ...
        opts = {
            ...
            float_opts = {
                ...
                width = function(_)
                    local col = vim.o.columns
                    return math.max(math.floor(col * 0.66 - 2), 120)
                end,
                height = function(_)
                    local row = vim.o.lines
                    return math.min(math.floor(row * 0.66 - 2), row - 6)
                end,
            },
        },
    },
